...
const KLY = (() => {
  ...
  function bind(){
    const ids = [
      "klySearch","klyLocateBtn","klyClearBtn","klyCityName","klyCitySub",
      "uvSummaryValue","uvSummaryRisk","pollenSummaryValue",
      "uvNow","uvMax","uvRisk","uvAdvice","uvChart",
  ...
  function getInitialState(){
    ...
    try{
      const prefer = localStorage.getItem("kly:preferLocation") === "1";
      if (prefer) return { ...DEFAULT, from:"preferLocation" };
    }catch{}
    try{
  ...
  async function geocode(query){
    ...
    function score(it){
      ...
      if (hasDiacritics){
        ...
        if (ns.startsWith(qStripped)) s += 40;
      } else {
        if (ns.startsWith(qStripped)) s += 90;
        if (nl.startsWith(q)) s += 60;
      }
      if (nl === q) s += 120;
      if (ns === qStripped) s += 110;
      return -s;
    }
  ...
  function renderSuggest(items){
    ...
    for (const it of items.slice(0,8)){
      ...
      b.addEventListener("click", () => { closeSuggest(); selectPlace({ name: it.name, admin1: it.admin1 || "", lat: it.lat, lon: it.lon }, { clearSearch:true, preferLocation:false }); });
  ...
  function wireSearch(){
    ...
    const syncClear = () => {
      if (els.searchWrap){
        els.searchWrap.classList.toggle("hasValue", (input.value||"").length>0);
      }
    };
    ...
    input.addEventListener("keydown", async (e) => {
      if (e.key === "Enter"){
        ...
        try{
          ...
          if (items.length){
            closeSuggest();
            selectPlace({ name: items[0].name, admin1: items[0].admin1 || "", lat: items[0].lat, lon: items[0].lon }, { clearSearch:true, preferLocation:false });
          }
        }catch{}
    ...
    if (els.klyClearBtn){
      els.klyClearBtn.addEventListener("click", () => {
        input.value = "";
        syncClear();
        closeSuggest();
        input.focus();
    ...
    if (els.klyLocateBtn) els.klyLocateBtn.addEventListener("click", () => useLocation(true));
  }

  async function useLocation(setPreferred){
    if (!navigator.geolocation){ selectPlace(DEFAULT, { preferLocation:false }); return; }
    if (setPreferred){
      try{ localStorage.setItem("kly:preferLocation","1"); }catch{}
    }
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        selectPlace({ name: best.name, admin1: "", lat: latitude, lon: longitude }, { clearSearch:true, preferLocation:true });
      },
      () => {
        if (setPreferred){
          try{ localStorage.setItem("kly:preferLocation","0"); }catch{}
        }
        selectPlace(DEFAULT, { preferLocation:false });
      },
  ...
  function selectPlace(place, opts={}){
    const { clearSearch=false, preferLocation=false } = opts;
    if (!preferLocation){
      try{ localStorage.setItem("kly:preferLocation","0"); }catch{}
    }
    ...
    setText(els.klyCityName, currentPlace.name);
    setText(els.klyCitySub, currentPlace.admin1 ? currentPlace.admin1 : "");

    if (clearSearch && els.klySearch){
      els.klySearch.value = "";
      if (els.searchWrap) els.searchWrap.classList.remove("hasValue");
      els.klySearch.blur();
      closeSuggest();
    }
  ...
  function init(){
    ...
    if (isHome && (initial.from === "default" || initial.from === "preferLocation")){
      if (initial.from === "preferLocation"){
        useLocation(false);
      } else {
        useLocation(false);
        selectPlace(initial, { preferLocation:false });
      }
    } else {
      selectPlace(initial, { preferLocation: (initial.from === "preferLocation") });
    }
  }
