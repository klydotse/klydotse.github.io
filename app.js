...
function initSaveAppPrompt() {
  ...
  const isSafari = /Safari/i.test(navigator.userAgent) && !/Chrome|CriOS|Edg|EdgiOS|OPR|OPiOS|FxiOS|Firefox/i.test(navigator.userAgent);

  function renderPanel(title, copy) {
    panel.innerHTML = `
      <p class="save-panel-title">${title}</p>
      <p class="save-panel-copy">${copy}</p>
  ...
  function togglePanel(title, copy) {
    const isHidden = panel.hidden;
    closePanel();
    if (isHidden) renderPanel(title, copy);
  }

  button.addEventListener('click', async () => {
    if (isStandalone()) {
      togglePanel('Redan sparad', 'Kly är redan sparad på din hemskärm eller installerad som app i webbläsaren.');
      return;
    }
    ...
    if (deferredPrompt) {
      ...
      renderPanel('Spara Kly', 'Du kan fortfarande spara Kly från webbläsarens meny om du vill lägga sidan på hemskärmen eller som bokmärke.');
      return;
    }

    if (isIOS && isSafari) {
      togglePanel('Spara Kly', 'I Safari: tryck på <strong>Dela</strong> och välj sedan <strong>Lägg till på hemskärmen</strong>.');
      return;
    }

    if (isIOS) {
      togglePanel('Spara Kly', 'På iPhone och iPad behöver du öppna sidan i <strong>Safari</strong> och sedan välja <strong>Dela</strong> → <strong>Lägg till på hemskärmen</strong>.');
      return;
    }

    togglePanel('Spara Kly', 'Öppna webbläsarens meny och välj <strong>Lägg till på hemskärmen</strong> eller <strong>Lägg till i bokmärken</strong>.');
  });
