...
function readRedirectedPath() {
  try {
    const params = new URLSearchParams(window.location.search || '');
    return params.get('kly_path') || '';
  } catch (_) {
    return '';
  }
}

const REDIRECTED_PATH = readRedirectedPath();
...
function getRoutingPath() {
  return sanitizeRelativePath(REDIRECTED_PATH || window.location.pathname || '/');
}

function getInitialSlug() {
  const path = getRoutingPath().split('#')[0].split('?')[0].replace(/\/+$/, '') || '/';
...
function renderPollen(rows) {
  ...
  const isActiveRow = (row) => row.slice(1, 1 + POLLEN_FORECAST_DAYS).some((code) => levelRank(code) > 0);

  const compareByForecastStrength = (a, b) => {
    for (let index = 1; index <= POLLEN_FORECAST_DAYS; index += 1) {
  ...
  sorted.forEach(([type, today, day2, day3]) => {
...
function getCurrentPlaceUrl() {
  const slug = state.slug || getInitialSlug() || 'stockholm';
...
function initShareButton() {
  ...
  let resetTimer = null;
  const defaultLabel = button.dataset.defaultLabel || 'Dela';

  function setButtonLabel(label) {
    button.textContent = label;
    button.setAttribute('aria-label', `${label} länk till vald plats`);
    if (label !== defaultLabel) {
      window.clearTimeout(resetTimer);
      resetTimer = window.setTimeout(() => {
        button.textContent = defaultLabel;
        button.setAttribute('aria-label', 'Dela länk till vald plats');
      }, 1800);
    }
  }

  button.addEventListener('click', async () => {
    const placeName = state.activePlace?.name || document.getElementById('placeName')?.textContent?.trim() || 'den här platsen';
    const url = getCurrentPlaceUrl();
    const shareData = {
      title: `Kly – ${placeName}`,
      text: `Kolla vädret för ${placeName} på Kly`,
      url
    };
    ...
    try {
      if (navigator.share) {
        ...
        setButtonLabel('Delad');
        return;
      }

      if (navigator.clipboard?.writeText) {
        await navigator.clipboard.writeText(url);
        setButtonLabel('Kopierad');
        return;
      }
    } catch (error) {
    ...
    window.prompt('Kopiera länken till platsen:', url);
  });
}
...
function coerceRelativePollenDateKey(key, dayOrder) {
  const normalized = normalizePollenKey(key);
  if (!normalized) return '';
  if (normalized === 'today' || normalized === 'idag') return dayOrder[0] || '';
  if (normalized === 'tomorrow' || normalized === 'imorgon') return dayOrder[1] || '';
  if (normalized === 'dayaftertomorrow' || normalized === 'iovermorgon') return dayOrder[2] || '';
  const match = normalized.match(/^day([1-9])$/);
  if (match) {
...
function countFilledPollenCells(rows) {
  return (rows || []).reduce((total, row) => total + row.slice(1).filter((code) => levelRank(code) > 0).length, 0);
}

function mergePollenBundles(bundles) {
  if (!Array.isArray(bundles) || !bundles.length) return null;
  const base = { ...bundles[0], rows: bundles[0].rows.map((row) => [...row]) };
  const rowMap = new Map(base.rows.map((row) => [row[0], row]));

  bundles.slice(1).forEach((bundle) => {
    (bundle?.rows || []).forEach((row) => {
      const existing = rowMap.get(row[0]);
      if (!existing) {
        rowMap.set(row[0], [...row]);
        return;
      }
      for (let index = 1; index < row.length; index += 1) {
        if (levelRank(row[index]) > levelRank(existing[index])) existing[index] = row[index];
      }
    });
    if (!base.summary && bundle?.summary) base.summary = bundle.summary;
  });

  base.rows = POLLEN_ROW_ORDER.map((type) => rowMap.get(type) || [type, '-', '-', '-']);
  return base;
}

function parsePollenForecastPayload(payload, meta, regionName, startDate) {
  const items = extractItems(payload);
  const dayOrder = Array.from({ length: POLLEN_FORECAST_DAYS }, (_, offset) => plusDays(startDate, offset));
  ...
  const registerEntry = (entry, fallbackDate = '', context = {}) => {
    ...
    const pollenId = firstString(merged, ['pollenId', 'pollen_id', 'pollenTypeId', 'pollen_type_id', 'pollenKey', 'typeId', 'type_id', 'id']);
    const rawName = firstString(merged, ['name', 'label', 'pollenName', 'pollen_name', 'type', 'code'])
      || meta.pollenTypes.get(pollenId)
      || meta.pollenTypes.get(String(pollenId || ''))
      || pollenId;
    ...
    const dateKey = coerceDateOnly(firstDefined(merged, ['date', 'time', 'forecast_date', 'forecastDate', 'start_date', 'startDate', 'valid_date', 'validDate', 'day']))
      || fallbackDate
  ...
  const walkNode = (node, fallbackDate = '', context = {}, depth = 0) => {
    ...
    if (Array.isArray(node)) {
      node.forEach((child, index) => {
        const derivedDate = coerceDateOnly(firstDefined(child, ['date', 'time', 'forecast_date', 'forecastDate', 'start_date', 'startDate', 'valid_date', 'validDate', 'day']))
          || fallbackDate
    ...
    Object.entries(node).forEach(([key, value], index) => {
      ...
      const normalizedKey = normalizePollenKey(key);
      if ([
        'levelseries', 'level_series', 'series',
        'values', 'levels', 'entries', 'items', 'data',
        'pollenlevels', 'pollen_levels',
        'currentvalues', 'current_values',
        'forecasts', 'forecast', 'days', 'daily',
        'summary', 'description', 'message', 'text', 'forecasttext', 'forecast_text',
        'date', 'time', 'forecastdate', 'forecast_date', 'startdate', 'start_date', 'validdate', 'valid_date', 'day'
      ].includes(normalizedKey)) return;

      const keyDate = coerceDateOnly(key) || coerceRelativePollenDateKey(key, dayOrder) || nodeDate;
      const pollenNameFromKey = canonicalPollenRowName(key) || canonicalPollenRowName(meta.pollenTypes.get(String(key) || ''));

      if (pollenNameFromKey) {
        const derivedEntry = safeObject(value) ? { pollenName: key, ...value } : { pollenName: key, level: value };
        if (registerEntry(derivedEntry, keyDate, nextContext)) parsedAny = true;
        if ((safeObject(value) || Array.isArray(value)) && walkNode(value, keyDate, { ...nextContext, pollenName: key, seriesIndex: index }, depth + 1)) parsedAny = true;
        return;
      }

      if (safeObject(value) || Array.isArray(value)) {
        const genericContext = { ...nextContext, seriesIndex: index };
        if (walkNode(value, keyDate, genericContext, depth + 1)) parsedAny = true;
      }
    });
...
async function fetchSwedenPollen(place) {
  ...
  const endDate = plusDays(startDate, POLLEN_FORECAST_DAYS - 1);
  const urls = [
  ...
  for (const url of urls) {
    try {
      ...
      if (!extractItems(candidate).length) continue;
      const parsed = parsePollenForecastPayload(candidate, meta, region.name, startDate);
      if (countFilledPollenCells(parsed.rows) > 0 || parsed.summary) parsedBundles.push(parsed);
    } catch (error) {
      lastError = error;
    }
  }
  ...
  parsedBundles.sort((a, b) => countFilledPollenCells(b.rows) - countFilledPollenCells(a.rows));
  const parsed = mergePollenBundles(parsedBundles) || parsedBundles[0];
...
function addPlaceToIndex(index, place) {
...
function rememberPlace(place) {
  addPlaceToIndex(state.placeIndex, place);
  return place;
}

function buildPlaceIndex(places) {
  const index = new Map();
  places.forEach((place) => addPlaceToIndex(index, place));
  return index;
}
...
async function resolvePlaceBySlug(slug) {
  const normalized = slugify(slug || '');
  if (!normalized || normalized === 'stockholm') {
    return state.placeIndex.get('stockholm') || state.places[0] || INITIAL_PLACE;
  }

  if (state.placeIndex.has(normalized)) return state.placeIndex.get(normalized);

  const remoteList = await searchRemotePlaces(humanizeSlug(normalized), { limit: 12 });
  const remoteMatch = remoteList.find((candidate) => {
    const urlSlug = candidate.url_slug || getPlaceSlug(candidate);
    const routeSlug = slugify(candidate.route_slug || '');
    const nameSlug = slugify(candidate.name || '');
    return urlSlug === normalized || routeSlug === normalized || nameSlug === normalized;
  });

  if (remoteMatch) return rememberPlace(remoteMatch);
...
function buildPollenRows(airData) {
  ...
  const days = [...dateBuckets.values()].slice(0, POLLEN_FORECAST_DAYS);
  while (days.length < POLLEN_FORECAST_DAYS) {
...
function buildUvDataFromAir(airData, { dateKey = '', timeZone = 'UTC' } = {}) {
  const wanted = ['06', '09', '12', '15', '18', '21'];
  const hourly = airData?.hourly || {};
  const uvSeries = Array.isArray(hourly.uv_index_clear_sky)
    ? hourly.uv_index_clear_sky
    : Array.isArray(hourly.uv_index)
      ? hourly.uv_index
      : [];

  if (!Array.isArray(hourly.time) || !hourly.time.length || !uvSeries.length) {
    return null;
  }

  const points = createUvPointSeries(wanted, hourly.time, uvSeries, { dateKey, timeZone });
  const peak = buildUvPeakOverride(hourly.time, uvSeries, { dateKey, timeZone });
  return {
    points,
    peak,
    hasData: points.some(([, value]) => Number(value) > 0) || peak.maxValue > 0
  };
}
...
function createLiveData(place, weatherData, airData, pollenBundle = null) {
  ...
  const airUvBundle = buildUvDataFromAir(airData, { dateKey: localDateKey, timeZone });
  const uv = airUvBundle?.hasData ? airUvBundle.points : fallbackUv;
  const uvPeak = airUvBundle?.hasData ? airUvBundle.peak : fallbackUvPeak;
...
async function fetchLiveData(place) {
  ...
  airUrl.searchParams.set('hourly', 'alder_pollen,birch_pollen,grass_pollen,mugwort_pollen,ragweed_pollen,uv_index,uv_index_clear_sky');
...
function renderDataForPlace(place, data, urlSlug, updateHistory = false) {
  ...
  setText('uvSummary', uvSummaryText);
...
async function renderPlace(place, updateHistory = false) {
  ...
  try {
    ...
    console.error(error);
    if (token !== state.renderToken) return;
    const fallback = FALLBACK_DATA[urlSlug] || FALLBACK_DATA[place.route_slug] || FALLBACK_DATA.stockholm;
...
async function initSearch() {
  ...
  input.addEventListener('input', async () => {
    ...
    merged.forEach((place) => {
      if (!state.placeIndex.has(place.url_slug || '')) rememberPlace(place);
    });
  ...
  if (initialSlug !== 'stockholm') {
    const initialPlace = await resolvePlaceBySlug(initialSlug);
...
window.addEventListener('popstate', async () => {
  const nextPlace = await resolvePlaceBySlug(getInitialSlug());
  renderPlace(nextPlace, false);
});
window.addEventListener('DOMContentLoaded', () => {
  if (REDIRECTED_PATH) {
    history.replaceState({ slug: getInitialSlug() }, '', sanitizeRelativePath(REDIRECTED_PATH));
  }
  ...
  renderPlace(INITIAL_PLACE, false);
  initSearch();
});
