...
const state = { slug: 'stockholm', selectedIndex: -1, results: [], places: [], placeIndex: new Map(), liveCache: new Map(), geocodeCache: new Map(), remoteSearchCache: new Map(), pollenForecastCache: new Map(), pollenMetaPromise: null, renderToken: 0, searchToken: 0, activePlace: null, searchCoreLoaded: false, searchAbroadLoaded: false, searchLoadPromise: null, searchPrefetchStarted: false };
...
async function fetchSearchSource(url) {
  ...
  throw new Error('Invalid place data format');
}

function refreshSearchIndex(places) {
  state.places = places.filter((place) => !place.search_hidden);
  state.placeIndex = buildPlaceIndex(state.places);
  if (window.KlySwedenSearch) {
    window.KlySwedenSearch._places = state.places;
    window.KlySwedenSearch._loaded = true;
  }
}

async function ensureSearchLoaded(mode = 'core') {
  if (mode === 'abroad' && state.searchAbroadLoaded) return state.places;
  if (mode !== 'abroad' && state.searchCoreLoaded) return state.places;
  if (state.searchLoadPromise) return state.searchLoadPromise;

  state.searchLoadPromise = (async () => {
    if (!state.searchCoreLoaded) {
      const corePlaces = (await Promise.all([
        fetchSearchSource('./resources/sweden-places.json'),
        fetchSearchSource('./resources/sweden-local-areas-seed.json'),
        fetchSearchSource('./resources/sweden-local-areas-backlog.json')
      ])).flat();
      refreshSearchIndex(corePlaces);
      state.searchCoreLoaded = true;
    }

    if (mode === 'abroad' && !state.searchAbroadLoaded) {
      const abroadPlaces = await fetchSearchSource('./resources/places-abroad-seed.json');
      const merged = state.places.concat(abroadPlaces);
      refreshSearchIndex(merged);
      state.searchAbroadLoaded = true;
    }
  ...
  try {
    return await state.searchLoadPromise;
  } finally {
    state.searchLoadPromise = null;
  }
}

function prefetchSearchCore() {
  if (state.searchPrefetchStarted || state.searchCoreLoaded) return;
  state.searchPrefetchStarted = true;
  const load = () => ensureSearchLoaded('core').catch((error) => console.warn('Search prefetch failed', error));
...
async function initSearch() {
  ...
  if (!input || !resultsEl) {
    const initialPlace = DEMO[getInitialSlug()] ? { name: DEMO[getInitialSlug()].name, route_slug: getInitialSlug() } : { name: 'Stockholm', route_slug: 'stockholm' };
  ...
  input.addEventListener('input', async () => {
    ...
    await ensureSearchLoaded('abroad');
    if (requestToken !== state.searchToken) return;

    const expandedLocalList = window.KlySwedenSearch.search(query, { limit: 8, minChars: 2 });
    if (expandedLocalList.length) {
      renderResults(expandedLocalList);
      return;
    }
  ...
  const initialPlace = DEMO[initialSlug]
    ? { name: DEMO[initialSlug].name, route_slug: initialSlug, url_slug: initialSlug }
    : { name: 'Stockholm', route_slug: 'stockholm', url_slug: 'stockholm' };
...
window.addEventListener('popstate', async () => {
  ...
  if (!state.searchCoreLoaded) {
    const fallbackPlace = DEMO[slug] ? { name: DEMO[slug].name, route_slug: slug, url_slug: slug } : { name: 'Stockholm', route_slug: 'stockholm', url_slug: 'stockholm' };
    renderPlace(fallbackPlace, false);
    prefetchSearchCore();
    return;
  }
  const nextPlace = resolvePlaceFromSlug(slug) || { name: 'Stockholm', route_slug: 'stockholm', url_slug: 'stockholm' };
